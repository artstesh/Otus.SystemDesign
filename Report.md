...
Оркестрационная сага с центральным координатором.
Применяется для оформления заказа, включающего резервирование товара, списание средств, вызов доставки.

Оркестратор получает команду PlaceOrder, последовательно вызывает сервисы платежей, доставки, обновляет статус. При сбое инициирует компенсации.

Для оркестрации Saga выбран асинхронный подход на базе Kafka: Orchestrator публикует команды в топики payment.commands и delivery.commands. Сервисы-участники потребляют команды и публикуют события успеха/отказа. Это обеспечивает высокую отказоустойчивость и развязывает сервисы во времени. Идемпотентность обработки гарантируется проверкой event_id.

Оркестратор Saga (реализованный как часть Order Service) спроектирован как stateless-сервис. Состояние каждой Saga сохраняется в таблицу saga_state в PostgreSQL (БД Order Service). Это позволяет масштабировать оркестратор горизонтально: любое количество экземпляров могут обрабатывать события и команды, читая и обновляя состояние через БД с использованием оптимистичных блокировок. При отказе одного из экземпляров другие продолжают обработку без потери данных.
![orchestration.png](Images/orchestration.png)

_Польза: оркестрация упрощает трассировку, централизует логику компенсаций и снижает связанность между сервисами._

### 2.6.3 Event Sourcing

В системе не используется Event Sourcing, так как это увеличило бы сложность и не требуется для предметной области. Для
заказов можно применить Event Sourcing для Order Service, записывая все события, однако, полагаю это излишним.
...
> Каждый микросервис владеет своей схемой данных. На диаграмме показана логическая модель предметной области, но физически таблицы распределены по разным базам данных.
...
> Нагрузка на PostgreSQL распределена между базами данных сервисов. Наиболее нагруженными являются Ad DB (запись объявлений) и Order DB (изменение статусов). Каждая база масштабируется независимо.
...
| CDN	            | Cache hit ratio	               | CDN           | 	Hit ratio < 0.9                  |
...
| При недоступности Elasticsearch	            | Логировать инцидент и инициировать восстановление, клиентам возвращать 299|
| При недоступности Redis	            | Кэширование отключается, все запросы идут в БД (увеличится latency, но система остаётся рабочей), алерт ответственным	 |
| При недоступности Payment Gateway	            | Заказы переводятся в статус payment_pending и ставятся в очередь повторных попыток с exponential backoff|
...
В системе используется полибаза данных: каждый микросервис владеет собственной базой данных PostgreSQL для обеспечения ACID и независимости. Это позволяет:
- Масштабировать сервисы независимо.
- Обеспечить отказоустойчивость: сбой одной БД не влияет на другие.

### 2.13.1. PostgreSQL

- Класс: Реляционная СУБД.
- Выбор: PostgreSQL.
- Масштабирование: шардирование для Ad DB (по регионам), партиционирование Order DB (по дате).
- Высокая доступность: Кластер с мастером и двумя синхронными репликами. Patroni для автоматического failover. PgBouncer
  для пула соединений.
- CAP: CP.
...
- TIA: Для каждого сервиса разворачивается своя схема/база, миграции управляются независимо.
...
- Индекс: 5 первичных шардов, 1 реплика.
- CAP: AP.
...
- Таблица orders партиционирована по диапазону created_at с ежемесячными партициями; старые партиции через 6 месяцев перемещать в табличное пространство на медленном диске или архивировать.
- Таблица ads шардирована по region_id. Инструмент - Citus
...
- Распределённая обработка: Elasticsearch и Redis кластеры распределены по зонам в пределах региона. Kafka также с
  репликацией между зонами.

## 3. Безопасность

### 3.1. Схема защиты и управления доступом

#### 3.1.1. Механизм аутентификации и авторизации

Ввыбран OAuth 2.0 + OpenID Connect (OIDC) с использованием JWT.

Архитектура выдачи и проверки токенов:

![auth.png](Images/auth.png)

Access token истекает → клиент отправляет refresh token на /auth/refresh. IP проверяет refresh token и выдаёт новую
пару.

Refresh token имеет абсолютный срок жизни; по истечении требуется повторная аутентификация.

#### 3.1.2. Безопасность передачи данных

SSL/TLS применяется для всех внешних и внутренних соединений:

Kafka, PostgreSQL, Redis, Elasticsearch также настроены на TLS.

Шифрование данных:

- Даже при защищённом канале чувствительные данные шифруются на уровне приложения:
- Пароли пользователей – хешируются bcrypt (соль + итерации) перед сохранением.
- Платёжные данные (PAN, CVV) – шифруются AES-256-GCM с ключами из Vault.
- Персональные данные (ФИО, адрес) – могут быть зашифрованы на уровне БД - вопрос к юр.отделу по поводу легальных
  рисков.

#### 3.1.3. Управление доступом (RBAC)

Реализована RBAC с возможностью расширения до ABAC в будущем.

Роли:

| Роль	    | Описание             | 	Примеры разрешений                                                                       |
|----------|----------------------|-------------------------------------------------------------------------------------------|
| user	    | Обычный пользователь | Чтение объявлений, оформление заказов, просмотр своих заказов.                            |
| seller   | 	Продавец            | 	Всё что у user, плюс управление своими объявлениями, просмотр заказов на свои товары.    |
| courier	 | Курьер               | 	Просмотр доступных заказов, обновление статуса доставки.                                 |
| admin	   | Администратор	       | Всё: модерация объявлений, управление пользователями, просмотр логов, изменение настроек. |
| service  | 	Сервисный аккаунт   | 	Доступ только к внутренним API, без прав пользователя.                                   |

Процесс добавления/изменения ролей:

- Роли и разрешения хранятся в PostgreSQL.
- Администратор через отдельный интерфейс может назначать роли пользователям.
- Изменения прав вступают в силу при следующей выдаче access token.

#### 3.1.4. Хранение и управление секретами

Инструмент: HashiCorp Vault развёрнут в кластере Kubernetes.

Что хранится в Vault:

- Пароли к БД.
- API ключи для платёжного шлюза, картографического сервиса, CDN.
- Приватные ключи для подписи JWT.
- Конфигурации приложений.

Схема безопасного хранения:

- Каждый микросервис при старте аутентифицируется в Vault.
- Vault выдаёт временный токен для доступа к секретам.
- Секреты монтируются в контейнер как переменные окружения.

Ротация секретов:

- Динамические секреты БД: Vault автоматически создаёт учётные записи с ограниченным TTL; приложения получают новые
  креды каждые N часов.
- JWT ключи: ротация каждые 30 дней, старые ключи остаются валидными ещё 24 часа для плавного перехода.

### 3.2. Аудит безопасности

#### 3.2.1. Механизмы логирования событий безопасности

Все события безопасности логируются в централизованную систему с обязательными полями:

- timestamp
- event_type (login, logout, token_refresh, access_denied, role_changed, secret_accessed, payment_attempt)
...
- Аутентификация: успешный/неуспешный вход, запрос refresh token, logout.
- Авторизация: отказ в доступе.
- Управление ролями: добавление/удаление роли у пользователя.
- Доступ к конфиденциальным данным: попытка чтения чужих заказов, платежей.
- Секреты: операции Vault.
- Административные действия: изменение настроек, модерация объявлений.

#### 3.2.2. Метрики для мониторинга аномального поведения

| Метрика                        | 	Описание	                                                  | Порог аномалии                          |
|--------------------------------|-------------------------------------------------------------|-----------------------------------------|
| failed_logins_rate             | 	Количество неудачных аутентификаций в минуту	              | > 10 за минуту с одного IP              |
| token_refresh_failures	        | Отказы в обновлении токенов           | 	> 5 за минуту                          |
| authz_denials_rate	            | Количество отказов доступа                            | > 20 за минуту                          |
| privilege_escalation_attempts	 | Попытки доступа к административным эндпоинтам от не-админов	 | > 5 за минуту                           |
| suspicious_ips                 | Запросы с IP из чёрных списков или с высоким процентом ошибок	 | Постоянный мониторинг                   |
| vault_operations_rate	         | Частота обращений к Vault               | 	Резкое увеличение может означать атаку |
| unauthorized_db_access         | 	Попытки подключения к БД без валидных кредов      | Любое появление                         |

#### 3.2.3. План реагирования на инциденты безопасности

|Событие | Действие                                                                                                          |
|---|-------------------------------------------------------------------------------------------------------------------|
|Массовые неудачные логины | Заблокировать IP (автоматически), проверить логи, уведомить администратора.                                       |
|Обнаружение утечки refresh token | Отозвать все refresh token для пользователя, потребовать повторную аутентификацию, отозвать токен. |
|Попытка доступа к чужим данным | Заблокировать сессию пользователя, провести расследование, при необходимости отозвать доступ.                     |
|Подозрительная активность из Vault | Проверить логи Vault, временно отключить доступ к секретам для подозрительного сервиса, сменить все секреты.      |

#### 3.2.4. Дополнительные меры

- Политика паролей: Минимальная длина 10 символов, сложность (заглавные, цифры, спецсимволы), хранение в bcrypt.
- Двухфакторная аутентификация: Планируется для администраторов и продавцов с высокими оборотами (в беклоге).

## 4. Доставка решения
### 4.1. Схема развертывания
#### 4.1.1. Применение Infrastructure as Code (IaC)
Для управления инфраструктурой выбран Terraform, что позволяет:
- Версионировать инфраструктуру.
- Разворачиваться по средам (dev, staging, prod).
...
#### 4.1.2. Развертывание системного и прикладного ПО
Ansible используется для конфигурации и для первоначальной настройки Kubernetes:
- Установка Docker, kubelet, kubeadm.
- Настройка network plugin (Calico).
- Установка инструментов мониторинга (Prometheus, Loki, Grafana).

Для микросервисов используется Kubernetes манифесты (Deployment, Service, Ingress, ConfigMap, Secret).

> Ingress помер, но, в данном случае, пока сам не переехал на другой подход, потому он останется здесь.
...
#### 4.1.3. Обновление ПО
- Обновление инфраструктуры: Изменения в Terraform применяются через CI/CD.
- Обновление прикладного ПО: Rolling update в Kubernetes для обеспечения нулевого простоя.
- Обновление баз данных: Миграции выполняются через специальный Job до деплоя новой версии сервиса.

### 4.2. CI/CD процесс
#### 4.2.1. Оперативная доставка релизов

Пайплайн включает этапы:
- Lint & Build
   - Проверка кода (dotnet format, eslint).
...
- Deploy to Kubernetes
   ...
   - Для разных окружений используются разные namespace и значения (Kustomize или Helm).
- Post-deploy Tests
   - Smoke‑тесты (проверка доступности API).
- Notifications
   - Отправка статуса в Slack.

#### 4.2.2. Обновления без простоев (Zero‑downtime)
- Rolling update в Kubernetes — стандартный механизм. Новые поды создаются до удаления старых.
- graceful shutdown.
- Миграции БД — выполняются перед деплоем новой версии сервиса; если миграция несовместима с предыдущей версией - blue‑green deployment.

Когда просто не получается:
- Изменение схемы БД, требующее блокировки.
- Изменение API, несовместимое с предыдущей версией.

Для таких случаев применяется blue‑green deployment:
- Два окружения.
- После проверки green переключается трафик.
- В случае проблем — быстрый откат переключением обратно.

#### 4.2.3. Observability
|Метрика| Описание                   |
|---|----------------------------|
|Deployment duration| Время от коммита до деплоя |
|Deployment failure rate	| 	Стабильность доставки     |
|Rollback count	| 	Частота откатов           |
|Pod restart rate	| 	Стабильность приложения   |

### 4.3. Планы тестирования
#### 4.3.1. Управление качеством решения
Подходы для повышения надёжности:

|Уровень|	Подход	|
|---|---|
|Код	|Unit‑тесты, статический анализ|
|API	|Контрактные тесты (Pact) между сервисами	|
|Интеграция	|Интеграционные тесты с реальными зависимостями |
|Безопасность	|SCA , SAST |
|E2E|	Тесты на staging |

Регламент:
- Code review обязателен для всех изменений.
- Прохождение всех CI‑проверок обязательно для слияния PR.
- На staging E2E и нагрузочные тесты перед релизом.
- Для prod — канареечное развертывание.
